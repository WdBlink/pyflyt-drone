...
class FixedwingObjLockEnv(FixedwingBaseEnv):
    ...
    def _ensure_fastsam_loaded(self) -> None:
        if self.fastsam_model is not None:
            return
        if not self.fastsam_weights_path or not os.path.exists(self.fastsam_weights_path):
            return
        try:
            FastSAM = self._safe_import_ultralytics_fastsam()
            self.fastsam_model = FastSAM(self.fastsam_weights_path)
        except Exception:
            self.fastsam_model = None
    ...
    def _try_patch_camera_capture(self):
        ...
        try:
            ...
            def capture_image(cam_self):
                ...
                try:
                    _, _, rgbaImg, depthImg, _ = cam_self.p.getCameraImage(
                ...
                seg = np.full((h, w, 1), -1, dtype=np.int32)
                self._ensure_fastsam_loaded()
                if self.fastsam_model is not None and hasattr(self, "duck_body_id"):
                    try:
                        inference_kwargs = {
                        ...
                        if self.bbox_prompt is not None:
                            inference_kwargs["bboxes"] = list(self.bbox_prompt)
                        elif self.fastsam_text_prompt:
                            inference_kwargs["texts"] = str(self.fastsam_text_prompt)

                        results = self.fastsam_model(rgba[..., :3], **inference_kwargs)
                        if results:
                            mask_np = self._fastsam_results_to_binary_mask(results[0])
                        else:
                            mask_np = None
                        ...
                        if mask_np is not None:
                            ...
                            seg[mask_np > 0] = self.duck_body_id
                    except Exception:
                        pass

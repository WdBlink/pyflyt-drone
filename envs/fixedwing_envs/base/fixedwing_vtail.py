...
class FixedwingVtail(DroneClass):
    """V tail Fixedwing instance that handles everything about a FixedWing."""
    ...
    def __init__(
        ...
        control_hz: int = 120,
        physics_hz: int = 240,
        drone_model: str = "fixedwing_vtail",
        ...
        with open(self.param_path, "rb") as f:
            ...
            surfaces.append(
                LiftingSurface(
                    ...
                    uav_id=self.Id,
                    surface_id=1,
                    lifting_unit=np.array([0.0, 1.0, 1.0]) / np.sqrt(1**2 + 1**2),
                    forward_unit=np.array([1.0, 0.0, 0.0]),
                    **all_params["left_ruddervator_params"],
                )
            )
            surfaces.append(
                LiftingSurface(
                    ...
                    uav_id=self.Id,
                    surface_id=2,
                    lifting_unit=np.array([0.0, -1.0, 1.0])
                    / np.sqrt(1**2 + 1**2),
                    forward_unit=np.array([1.0, 0.0, 0.0]),
                    **all_params["right_ruddervator_params"],
                )
            )
            surfaces.append(
                LiftingSurface(
            ...
            # V 尾混控参数（来自 yaml，可配置俯仰/偏航增益与是否限幅）
            vtail_mix_params = all_params.get("vtail_mix_params", {})
            self.vtail_pitch_gain = float(vtail_mix_params.get("pitch_gain", 1.0))
            self.vtail_yaw_gain = float(vtail_mix_params.get("yaw_gain", 1.0))
            self.vtail_clip = bool(vtail_mix_params.get("clip", True))
            # 6x4 混控矩阵：行对应[左副翼, 右副翼, 左V尾, 右V尾, 主翼, 推力]，列对应[Roll, Pitch, Yaw, Thrust]
            self.surface_mix = np.array(
                [
                    [1.0, 0.0, 0.0, 0.0],  # 左副翼 = +Roll
                    [-1.0, 0.0, 0.0, 0.0],  # 右副翼 = -Roll
                    [0.0, self.vtail_pitch_gain, self.vtail_yaw_gain, 0.0],  # 左V尾 = Pitch + Yaw
                    [0.0, self.vtail_pitch_gain, -self.vtail_yaw_gain, 0.0],  # 右V尾 = Pitch - Yaw
                    [0.0, 0.0, 0.0, 0.0],  # 主翼不参与 RPY
                    [0.0, 0.0, 0.0, 1.0],  # 推力 = Thrust
                ],
                dtype=np.float64,
            )
    ...
    def update_control(self, physics_step: int) -> None:
        ...
        # the default mode
            ...
            self.cmd = self.surface_mix @ self.setpoint
            if self.vtail_clip:
